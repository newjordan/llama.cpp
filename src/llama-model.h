...
struct llama_model {
    ...
    // psycho: single norm over the fused target context (applied once in the encoder pass)
    struct ggml_tensor * hidden_norm = nullptr;

    // unified vector to store target-model extracted layer ids in eagle3, psycho, etc.
