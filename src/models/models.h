...
struct llama_model_psycho : public llama_model_base {
    llama_model_psycho(const struct llama_model_params & params) : llama_model_base(params) {}
    void load_arch_hparams(llama_model_loader & ml) override;
    void load_arch_tensors(llama_model_loader & ml) override;

    // is_enc=true  : feature-fusion pass  (concat target hiddens -> fc -> hidden_norm -> context g)
    // is_enc=false : block-draft decoder  (block tokens cross+self attend over [context || block])
    template <bool is_enc>
    struct graph : public llm_graph_context {
        graph(const llama_model & model, const llm_graph_params & params);
