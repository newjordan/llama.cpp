...
# RC8+: extra ship-env vars for the candidate arm only (space-separated
# KEY=VAL list), e.g. TREEBEARD_PCBT_ENABLE=1 — gate exactly what ships.
if [[ -n "${TREEBEARD_CANDIDATE_EXTRA_ENV:-}" ]]; then
    read -r -a CANDIDATE_EXTRA <<< "$TREEBEARD_CANDIDATE_EXTRA_ENV"
    CANDIDATE_ENV+=("${CANDIDATE_EXTRA[@]}")
fi
