...
def arm_stats(doc):
    tps = [s["branch_wave"]["aggregate_predicted_tps"] for s in doc["samples"]]
    hashes = [tuple(sorted(r["tokens_sha256"] for r in s["branch_wave"]["requests"]))
              for s in doc["samples"]]
    return {
        "aggregate_tps_mean": statistics.fmean(tps),
        "aggregate_tps_p50": statistics.median(tps),
        "samples": len(tps),
...
def main():
    ...
    failures = sum(arms[l]["failed_samples"] for l in arms)
    ...
    summary = {
        ...
        "arm_p50_tps": {l: round(v, 6) for l, v in p50.items()},
        "arm_mean_tps": {l: round(arms[l]["aggregate_tps_mean"], 6) for l in arms},
