#!/usr/bin/env bash
# B4 slot-arbitration window: guarded bench server + the B4 driver.
set -Eeuo pipefail
...
LIVE_PORT=8093
BENCH_PORT=8098
DURATION="${TREEBEARD_B4_DURATION:-90}"
RUN_ID="$(date +%Y%m%d-%H%M%S)-b4"
OUT="$ROOT/results/treebeard-b4/$RUN_ID"
STARTED="$(date --iso-8601=seconds)"
BENCH_PID=
mkdir -p "$OUT/maintenance"
mkdir -p "$ROOT/results/treebeard-b4"
printf '%s\n' "$OUT" > "$ROOT/results/treebeard-b4/latest-run.txt"
...
restore_production() {
    ...
    if [[ -n "${BENCH_PID:-}" ]] && kill -0 "$BENCH_PID" 2>/dev/null; then
        kill "$BENCH_PID"; for _ in {1..60}; do kill -0 "$BENCH_PID" 2>/dev/null || break; sleep 1; done
        kill -9 "$BENCH_PID" 2>/dev/null; wait "$BENCH_PID" 2>/dev/null
    fi
    systemctl --user start "$SERVICE" || ok=0
    for _ in {1..240}; do curl -fsS --max-time 3 "http://127.0.0.1:$LIVE_PORT/health" >/dev/null 2>&1 && break; sleep 1; done
...
env TREEBEARD_PCBT_ENABLE=1 \
    ...
    --host 127.0.0.1 --port "$BENCH_PORT" --jinja --metrics -a b4 \
...
python3 "$WORKTREE/scripts/treebeard-b4-arbitration.py" \
    --port "$BENCH_PORT" --duration "$DURATION" --loads 4,8,11 --out "$OUT" \
    | tee "$OUT/b4-console.log" || true
